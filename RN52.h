/*
	RN52.cpp
	Adapted from the espsoftwareserial library by plerup. https://github.com/plerup/espsoftwareserial
...
public:
	RN52(int receivePin, int transmitPin, bool inverse_logic = false, int bufSize = 64, int isrBufSize = 0);
	virtual ~RN52();
#ifndef ESP32
	// Returns false if more than RN52_MAX_INSTS instances are started
	bool begin(int32_t baud) {
		return begin(baud, SWSERIAL_8N1);
	}
	bool begin(int32_t baud, RN52Config config);
#else
	void begin(int32_t baud) {
		begin(baud, SWSERIAL_8N1);
	}
	void begin(int32_t baud, RN52Config config);
#endif
	int32_t baudRate();
